                ...
                <table class="table-auto border-collapse w-full rounded-md">
                    <thead class="text-white">
                        <tr class="bg-gray-500 text-center">
                            ...
                            <th class="px-4 py-2">Subbab File</th>
                            <th class="px-4 py-2">Jenis File</th>
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white divide-y divide-gray-400">
                            ...
                            <td class="py-1">{{ $security->title }}</td>
                            <td class="py-1">{{ $security->subbab }}</td>
                            <td class="py-1">{{ $security->category }}</td>
                            <td class="text-center py-1">
                                <div class="flex justify-center space-x-2">
                                    <!-- Tombol View -->
                                    <a href="{{ route('showSecurity', $security->id) }}">
                                    ...
                                    <!-- Tombol Edit -->
                                    <form action="{{ route('editSecurity', $security->id) }}" method="GET">
                                        @csrf
                                    ...
                                    <!-- Tombol Delete -->
                                    <form action="{{ route('deleteSecurity', $security->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
